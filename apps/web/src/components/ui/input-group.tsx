...
const inputGroupButtonVariants = cva('flex items-center gap-2 rounded-md text-xs/relaxed shadow-none', {
  variants: {
    size: {
      xs: 'h-6 gap-1 rounded-[calc(var(--radius-sm)-2px)] px-2 [&>svg:not([class*=\'size-\'])]:size-3.5',
      sm: 'h-7 gap-1 px-2 text-xs [&>svg:not([class*=\'size-\'])]:size-3.5',
      'icon-xs': 'size-6 p-0 has-[>svg]:p-0 [&>svg:not([class*=\'size-\'])]:size-3.5',
      'icon-sm': 'size-7 p-0 has-[>svg]:p-0 [&>svg:not([class*=\'size-\'])]:size-4',
    },
  },
  defaultVariants: {
    size: 'xs',
...
function InputGroupInput({ className, ...props }: ComponentProps<typeof Input>) {
  return (
    <Input
      ...
      className={cn(
        'h-auto flex-1 rounded-none border-0 bg-transparent p-0 shadow-none ring-0 focus-visible:outline-0 focus-visible:ring-0 aria-invalid:ring-0 dark:bg-transparent',
        className,
      )}
      {...props}
    />
