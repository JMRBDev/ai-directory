  ...
  return (
    <>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_11rem_11rem_13rem]">
        <Field>
          ...
          <InputGroup className="mt-2">
            ...
            <InputGroupInput id="resource-search" type="search" placeholder="Name, owner, or description" value={query} onChange={(event) => onQueryChange(event.target.value)} />
          </InputGroup>
        </Field>
        <Field>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t pt-3 text-xs text-muted-foreground">
        <span>{filteredCount === 0 ? 'No resources found' : `Showing ${(currentPage - 1) * pageSize + 1}-${Math.min(currentPage * pageSize, filteredCount)} of ${filteredCount}`}</span>
